...
def run_crawler(github_token: Optional[str] = None, 
    ...
    # Process repositories immediately
    for idx, repo in enumerate(filtered_repos[:max_results]):
        # Skip if max results is reached
        ...
        if analyze_with_ai and analyzer:
            print(f"Analyzing repository {len(output_data)+1}/{min(len(filtered_repos), max_results)}: {repo.get('name')}")
            try:
        ...
        if len(output_data) % 5 == 0:
            try:
                ...
                print(f"Saved progress: {len(output_data)} repositories analyzed")
            except Exception as e:
